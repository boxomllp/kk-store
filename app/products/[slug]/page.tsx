    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setProduct(p);
          if (p.variants?.enabled) {
            const def = p.variants.options.find((o) => o.default) || p.variants.options[0];
  ...
  if (!product) {
    return (
      <>
        <AnnouncementBar />
        <Header />
        <div className="max-w-6xl mx-auto px-4 py-20 text-center text-gray-500">Product not found.</div>
        <Footer />
      </>
    );
  }
